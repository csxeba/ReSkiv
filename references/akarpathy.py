...
def sigmoid(z):
    return 1.0 / (1.0 + np.exp(-z))  # sigmoid "squashing" function to interval [0,1]
...
def policy_forward(inputs):
    hidden_state = np.dot(model['W1'], inputs)
    hidden_state[hidden_state < 0] = 0  # ReLU nonlinearity
    logp = np.dot(model['W2'], hidden_state)
    p = sigmoid(logp)
    return p, hidden_state  # return probability of taking action 2, and hidden state


def policy_backward(ep_hiddens, ep_dlogprob):
    """ backward pass. (eph is array of intermediate hidden states) """
    dW2 = np.dot(ep_hiddens.T, ep_dlogprob).ravel()
    dh = np.outer(ep_dlogprob, model['W2'])
    dh[ep_hiddens <= 0] = 0  # backpro prelu
    dW1 = np.dot(dh.T, epx)
